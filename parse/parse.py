	...
	if (b.firstChild and b.getElementsByTagName('text')[0].firstChild.nodeValue == "("): #if this is a question. Note that the character of choice is the (
		...
		for i in range(len(question)):
			...
			if (letter == "\n"):
				...
				qtext.append(letter)
			if 'size' in question[i].attributes.keys():
			...
			if (not ('size' in question[i].attributes.keys()) and answerState and i < len(question)-1): #if it's a space, we're in the answer section, and not at the end
				if ('size' in question[i+1].attributes.keys() and float(question[i+1].attributes['size'].value) < 14 and len(answer) > 0): #check if the next char is bolded
